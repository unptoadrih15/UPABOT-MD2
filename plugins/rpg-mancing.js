...
${ usedPrefix }Cook catfish `    
        ...
        conn.reply( m.chat, mcng, m)
         ...
         conn.sendButton( m.chat, `Has conseguido 🎁chest weapons epic ${psepick} item`, wm, [[`De nuevo`, `.mancing`]], m)
         } 
        if  (legendary > 0 ) {
        	global.db.data.users[m.sender].legendary += legendary * 1
        conn.sendButton( m.chat, `You Get 🎁chest weapons ${legendary} item`, wm, [[`De nuevo`, `.mancing`]], m)
        	}
        }, 38000)

        setTimeout(() => {
                     m.reply(`*El pez mordió del anzuelo y tú intentas tirar de él.*`)
                      }, 28000)
        setTimeout(() => {
                     m.reply(`*Esperando a que pique el pez*`)
                      }, 18000)
       setTimeout(() => {
                     m.reply(`*Has tirado un anzuelo al río*`)
                      }, 8000)
       setTimeout(() => {
                     m.reply(`*Vas a pescar🎣*`)
                      }, 0)
        ...
        global.db.data.users[m.sender].nila += nila * 1
         ...
         user.kepiting += kepiting * 1
        	  ...
        	  } else m.reply(`Ya has pescado anteriormente, por favor espera ${timers}`)
           } else conn.sendButton(m.chat, `*[❗] No tienes una caña de pescar 🎣*`, wm, [[`Craft Fishingrod`, '.craft pancing']], m)
           } catch (e) {
        console.log(e)
...
handler.tags = ['rpg']
handler.command = /^(pesca|pescar|fish)$/i 
